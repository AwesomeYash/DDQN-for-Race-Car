...
# This class implements a Double Deep Q-Network (DDQN) agent
class DDQNAgent(object):
    def __init__(self, alpha, gamma, n_actions, epsilon, batch_size,
                 input_dims, epsilon_dec=0.9995,  epsilon_end=0.10,
                 mem_size=25000, fname="C:\\Users\\yashr\\Desktop\\DDQN-Car-Racing\\ddqn_model_track.h5", replace_target=25):     #used frame = ddqn_model.h5 for batch size=128
    ...
    # This function is called to remember the state, action, reward, new state, and done flag
    def remember(self, state, action, reward, new_state, done):
        self.memory.store_transition(state, action, reward, new_state, done)

    # This function is called to choose an action based on the current state
    ...
    # Learning function to update the Q-values
    def learn(self):
    ...
    # This function is called to update the target network parameters
    ...
    def load_model(self):
        ...
        # No need to recompile for inference
        if self.epsilon == 0.0:
            self.update_network_parameters()

# Brain class to create the neural network model
class Brain:
    ...
    def createModel(self):
        ...
        model.add(tf.keras.layers.Dense(128, activation=tf.nn.relu)) #prev 128 
    ...
    # This function is called to predict the Q-values for a given state
    def predict(self, s):
        return self.model.predict(s)

    # This function is called to predict the Q-values for a single state
    def predictOne(self, s):
        return self.model.predict(tf.reshape(s, [1, self.NbrStates])).flatten()
    
    # This function is called to copy the weights from one model to another

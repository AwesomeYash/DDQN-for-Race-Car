...
import pygame
import numpy as np
from ddqnAgentTrack import DDQNAgent
...
def run():
    for e in range(N_EPISODES):
        # Reset the game environment for each episode
        game.reset()    
        ...
        renderFlag = False  # Flag to control rendering

        # Render every 50 episodes
        if e % 50 == 0 and e > 0: 
            renderFlag = True

        # To quit the game when done
        while not done:
        ...
        # Update the target network every REPLACE_TARGET episodes

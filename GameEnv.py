...
Description: This file contains the implementation of the Car Racing environment for a complex track using Pygame.
"""

# Import necessary libraries
import pygame
import math
from WallsTrack import Wall
from WallsTrack import getWalls
from GoalsTrack import Goal
from GoalsTrack import getGoals
...
# Function to rotate a rectangle defined by four points around its center by a given angle
...
# Class to represent a line segment defined by two points    
class myLine:
    def __init__(self, pt1, pt2):
        self.pt1 = myPoint(pt1.x, pt1.y)
        self.pt2 = myPoint(pt2.x, pt2.y)

# Class to make rays for collision detection
class Ray:
    ...
    # Function to cast a ray and check for intersection with a wall
    def cast(self, wall):
        x1 = wall.x1 
        y1 = wall.y1
        x2 = wall.x2
...
# Car class representing the car in the game
class Car:
    def __init__(self, x, y):
        self.pt = myPoint(x, y)
        self.x = x
        ...
        self.velX = 0
        self.velY = 0
        self.maxvel = 15    # before 18

        self.angle = math.radians(180)
        self.soll_angle = self.angle

        # Car position and rotation points
    ...
    # Function to set the action for the car based on the choice
    def action(self, choice):
        ...
        # Additional actions for more complex maneuvers 
        """
        elif choice == 5:
            self.accelerate(-self.dvel)
            self.turn(1)
    ...
    def accelerate(self,dvel):
        dvel = dvel * 2     # Multiples of 2 for faster acceleration
    ...
    # Fucntion to turn the car
    def turn(self, dir):
        self.soll_angle = self.soll_angle + dir * math.radians(15)
    
    # Function to update the car's position and velocity
    def update(self):
        ...
        elif(self.soll_angle < self.angle):
            if(self.soll_angle < self.angle - math.radians(10) * self.maxvel / ((self.velX**2 + self.velY**2)**0.5 + 1)):
                self.angle = self.angle - math.radians(10) * self.maxvel / ((self.velX**2 + self.velY**2)**0.5 + 1)
            else:
                self.angle = self.soll_angle
    ...
    # Function to cast rays from the car to detect walls and obstacles
    def cast(self, walls):
    ...
    # Score function to check if the car has passed a goal/checkpoint
    def score(self, goal):
    ...
    # Function to reset the car's position and state
    def reset(self):
        self.x = 50
        self.y = 300
        self.velX = 0
    ...
    # Draw function to render the car on the screen
    def draw(self, win):
        win.blit(self.image, self.rect)
  
# Racing environment class
# This class encapsulates the game environment, including the car, walls, and goals.
class RacingEnv:
    def __init__(self):
        pygame.init()
        ...
        self.back_image = pygame.image.load("C:\\Users\\yashr\\Desktop\\DDQN-Car-Racing\\track.png").convert()
    ...
    # Function to reset the game environment
    def reset(self):
        self.screen.fill((0, 0, 0))
    ...
    # Fucntion to step through the game environment
